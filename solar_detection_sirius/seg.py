...
from constants import DIGITIZATION_SIZE, IMGSZ, Y_IMGSZ, SEGMENT_COUNT, PATH_FITS, PATH_TMP
import numpy as np
...
def get_len(fits_file, path_fits):
    shutil.copy(path_fits, PATH_TMP)
    FitsConverter(PATH_TMP, PATH_TMP + r'\trash.csv', '', PATH_TMP, verbose=False).convert()
    len_line = 0
    os.chdir(PATH_TMP)
